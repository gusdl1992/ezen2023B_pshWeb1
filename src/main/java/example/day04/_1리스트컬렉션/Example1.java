    ...
    public static void main(String[] args) {
        ...
        // - ArrayList , Vector , LinkedList 클래스
            // 동기화 : 여러 스레드가 하나의 함수를 동시에 호출했을때 호출순서대로 해당 함수를 끝날때까지 점유 상태
                // 1인실 공부방 , 1인실
            // ArrayList : [배열] 동기화 지원X , 주로 단일스레드 사용
            // Vector : [배열] 동기화 지원 O , 주로 멀티스레드 사용
            // LinkedList : [연결구조] 동기화 지원 X , 별도의 추가 라이브러리 이용하면 동기화 가능
                // - 순차적인 삽입은 ArrayList 권장
                // - 빈번한 중간/특정 위치 삽입 과 삭제는 LinkedList 권장.
        ...
        // 순회
            // 1. for (int i = 0 ; i < 리스트명.size() ; i++){  }
            // 2. for (타입 반복변수명 : 리스트명){ 실행문 }
            // 3. 리스트명.forEach( 반복변수명 -> 실행문 );
                // forEach 실행문이 여러개일때 :  리스트명.forEach(반복변수명 -> { 실행문; 실행문2; } )
        ...
        // 1. 일반 for 문 : i 가 인덱스 역할을 한다.
        System.out.println("=== 일반 FOR 문 리스트 순회 ===");
        ...
        // 2. 향상된 for 문      for( 타입 반복면수명 : 리스트/배열명 ){} : 리스트내 순차적으로 하나씩 객체를 반복변수에 대입.
        System.out.println("=== 향상된 FOR 문 리스트 순회 ===");
        // 향상된 for 문 은 인덱스 가 없어 인덱스를 쓰고 싶으면 int 변수 선언 해서 저장
        int ch = 0;
        for (Board b :list){
        ...
        // forEach( 반복변수 -> 실행문 )
            // 리스트명.forEach(반복변수명 -> { 실행문; 실행문2; } ) 실행문이 여러개일때.
