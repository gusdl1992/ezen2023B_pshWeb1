...
public class BoardDao extends Dao {
    ...
    public boolean doUpdateBoard(BoardDto boardDto){
        ...
        try {
            ...
            System.out.println("BoardDao.doUpdateBoard : e = " + e);
        }
        return false;
    }
    ...
    public boolean doDeleteBoard( @RequestParam int bno){
        ...
        try {
            ...
            System.out.println("BoardDao.doDeleteBoard e = " + e);
        }
        return false;
    }
    ...
    public  boolean boardWriterAuth(long bno , String mid){
        try {
            String sql = "select * from board b inner join member m " +
                    " on b.mno = m.no where b.bno = ? and m.id = ?";
            ps = conn.prepareStatement(sql);
            ps.setLong(1,bno);
            ps.setString(2,mid);
            rs = ps.executeQuery();
        ...
        try {
            ...
            while (rs.next()){
                ...
                ps.setInt(1, Integer.parseInt(rs.getString("brno") ) );
                    // ( int ) : 캐스팅 = 부모,자식관계이여야한다. - Int 와 String 상하관계 아니다.(형변환) vs Integer.parseInt() : 형번환 함수
                    // ****************** rs 사용 하면 안되는 이유 : 현재 상위 댓글 출력시 rs 사용중 ( while (rs.next() ) )
